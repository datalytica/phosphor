 ...
 */
export
class SchemaList extends Widget {
  /**
   * Construct a new command palette.
   *
   * @param options - The options for initializing the palette.
   */
  constructor() { //options: SchemaList.IOptions) {
    super({ node: Private.createNode() });
    this.addClass('p-SchemaList');
    this.setFlag(Widget.Flag.DisallowLayout);
    //this.perspective = options.perspective;
    //this.renderer = options.renderer || SchemaList.defaultRenderer;
    this.renderer = SchemaList.defaultRenderer;
  }
   ...
   */
  readonly perspective: any;

  /**
   * The renderer used by the command palette.
   */
  readonly renderer: SchemaList.IRenderer;
  ...
  get searchNode(): HTMLDivElement {
    return this.node.getElementsByClassName('p-SchemaList-search')[0] as HTMLDivElement;
  }
  ...
  get inputNode(): HTMLInputElement {
    return this.node.getElementsByClassName('p-SchemaList-input')[0] as HTMLInputElement;
  }
  ...
  get contentNode(): HTMLUListElement {
    return this.node.getElementsByClassName('p-SchemaList-content')[0] as HTMLUListElement;
  }

  /**
   * A read-only array of the command items in the palette.
   */
  get items(): ReadonlyArray<SchemaList.IItem> {
    return this._items;
  }
   ...
   */
  addItem(options: SchemaList.IItemOptions): SchemaList.IItem {
   ...
   */
  removeItem(item: SchemaList.IItem): void {
  ...
  /**
   * Handle the `'p-dragenter'` event for the schemalist
   */
  ...
  /**
   * Handle the `'p-dragleave'` event for the schema list.
   */
  ...
  /**
   * Handle the `'p-dragover'` event for the schema list.
   */
  ...
  /**
   * Handle the `'mousedown'` event for the schemalist
   */
  private _evtMouseDown(event: MouseEvent): void {
    ...
    // Bail if the index is a header.
    if (part.type === 'header') {
      return;
    }

    let node = this.contentNode.children[index];
    let columnName = node.textContent;
    ...
    //let dragImage = drag.cloneNode(true) as HTMLElement;
    //dragImage.classList.add('p-SchemaList');
    ...
    body.classList.add('p-mod-column-dragging');

    // Create the cleanup callback.
    let cleanup = (() => {
      this._drag = null;
      body.classList.remove('p-mod-column-dragging');
    });


    // Start the drag operation and cleanup when done.
    this._drag.start(event.clientX, event.clientY).then(cleanup);
  }
  ...
  private _items: SchemaList.IItem[] = [];
...
/**
 * The namespace for the `SchemaList` class statics.
 */
export
namespace SchemaList {
  ...
  class Renderer implements IRenderer {
    ...
    renderHeader(data: IHeaderRenderData): VirtualElement {
      ...
      return h.li({ className: 'p-SchemaList-header' }, content);
    }
    ...
    renderEmptyMessage(data: IEmptyMessageRenderData): VirtualElement {
      ...
      return h.li({ className: 'p-SchemaList-emptyMessage' }, content);
    }
    ...
    renderItemLabel(data: IItemRenderData): VirtualElement {
      ...
      return h.div({ className: 'p-SchemaList-itemLabel' }, content);
    }
    ...
    renderItemCaption(data: IItemRenderData): VirtualElement {
      ...
      return h.div({ className: 'p-SchemaList-itemCaption' }, content);
    }
...
namespace Private {
  ...
  function createNode(): HTMLDivElement {
    ...
    search.className = 'p-SchemaList-search';
    wrapper.className = 'p-SchemaList-search-wrapper';
    icon.className = 'p-SchemaList-searchIcon';
    input.className = 'p-SchemaList-input';
    content.className = 'p-SchemaList-content';
   ...
   */
  export
  function createItem(options: SchemaList.IItemOptions): SchemaList.IItem {
  ...
  interface IItemResult {
     ...
     */
    readonly item: SchemaList.IItem;
   ...
   */
  export
  function search(items: SchemaList.IItem[], query: string): SearchResult[] {
  ...
  interface IScore {
     ...
     */
    item: SchemaList.IItem;
  }

  /**
   * Perform a fuzzy match on an array of command items.
   */
  function matchItems(items: SchemaList.IItem[], query: string): IScore[] {
   ...
   */
  function fuzzySearch(item: SchemaList.IItem, query: string): IScore | null {
  ...
  /**
   * A concrete implementation of `SchemaList.IItem`.
   */
  class CommandItem implements SchemaList.IItem {
    /**
     * Construct a new command item.
     */
    constructor(options: SchemaList.IItemOptions) {
